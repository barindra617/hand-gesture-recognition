...
def draw_dashboard(frame, gesture, confidence, fps):
    ...
    panel_width, panel_height = min(width - 32, 620), 142
    ...
    cv2.putText(frame, "HAND GESTURE RECOGNITION", (panel_x + 22, panel_y + 34),
                font, 0.72, white, 2, cv2.LINE_AA)
    cv2.putText(frame, f"Gesture: {gesture.upper()}", (panel_x + 22, panel_y + 68),
                font, 0.72, green, 2, cv2.LINE_AA)
    cv2.putText(frame, f"Confidence: {confidence * 100:.1f}%", (panel_x + 22, panel_y + 102),
                font, 0.62, white, 2, cv2.LINE_AA)
    cv2.putText(frame, f"FPS: {fps:.1f}", (panel_x + panel_width - 165, panel_y + 102),
                font, 0.62, white, 2, cv2.LINE_AA)

    bar_left, bar_top = panel_x + 22, panel_y + 118
    bar_right, bar_bottom = panel_x + panel_width - 22, panel_y + 130
...
while True:
    ...
    box_size = min(300, w, h)
    x1 = (w - box_size) // 2
    dashboard_bottom = 16 + 142
    y1 = min(max((h - box_size) // 2, dashboard_bottom + 18), h - box_size)
